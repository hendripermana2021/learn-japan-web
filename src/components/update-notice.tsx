  ...
  return (
   <>
      <button
        ...
        className="fixed bottom-4 left-4 z-70 inline-flex items-center gap-2 rounded-full border border-(--brand)/25 bg-(--paper) px-4 py-2 text-xs font-semibold tracking-[0.08em] text-(--brand) shadow-[0_14px_35px_-18px_rgba(0,0,0,0.7)] transition-transform duration-200 hover:-translate-y-0.5"
      >
      ...
      {isOpen ? (
        <div className="fixed inset-0 z-80 flex items-end justify-center bg-[linear-gradient(to_top,rgba(0,0,0,0.55),rgba(0,0,0,0.2))] p-4 sm:items-center">
          <div className="w-full max-w-xl overflow-hidden rounded-3xl border border-(--border-subtle) bg-(--paper) shadow-[0_36px_90px_-42px_rgba(0,0,0,0.85)]">
            <div className="bg-[radial-gradient(circle_at_top_left,var(--brand)_0%,transparent_55%),linear-gradient(to_right,var(--surface-panel-soft),transparent)] px-5 py-4">
              <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-(--brand)">
                <span className="rounded-full border border-(--brand)/30 bg-(--paper)/65 px-2 py-1">
                  {text.versionLabel} {UPDATE_VERSION}
                </span>
                <span className="rounded-full border border-(--brand)/20 bg-(--paper)/65 px-2 py-1 text-(--ink-soft)">
                  {text.dateLabel}: {text.releaseDate}
                </span>
              </div>
              <p className="mt-3 text-sm font-semibold uppercase tracking-[0.16em] text-(--brand)">{text.title}</p>
            ...
            <div className="px-5 pb-5 pt-4">
              ...
              {text.items.map((item) => (
                <li key={item} className="flex items-start gap-2 rounded-xl border border-(--border-subtle) bg-(--surface-panel-soft) px-3 py-2.5">
                  <span className="mt-1 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-(--brand)" aria-hidden>
              ...
              <div className="mt-5 flex justify-end">
                <button
                  type="button"
                  onClick={closeNotice}
                  className="rounded-xl bg-(--interactive-bg) px-4 py-2 text-sm font-semibold text-(--interactive-foreground) shadow-[0_14px_35px_-22px_rgba(0,0,0,0.8)] transition-transform duration-200 hover:-translate-y-0.5"
                >
                  {text.close}
                </button>
              </div>
